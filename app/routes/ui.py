...
def index(request: Request) -> HTMLResponse:
    return templates.TemplateResponse("index.html", {"request": request, "error": None, "form": {}})


@router.get("/history", response_class=HTMLResponse)
def history(request: Request, session: Session = Depends(get_session)) -> HTMLResponse:
    records = session.exec(select(AnalysisRecord).order_by(desc(AnalysisRecord.created_at)).limit(20)).all()
...
async def analyze(
    ...
    try:
        ...
        return templates.TemplateResponse("index.html", {"request": request, "error": exc.detail, "form": form_state}, status_code=exc.status_code)
    except Exception:
        logger.exception("Unexpected error during analysis")
        return templates.TemplateResponse(
            "index.html",
            {"request": request, "error": "Unexpected error while analyzing audio. Please verify FFmpeg is installed and try again.", "form": form_state},
            status_code=500,
        )
